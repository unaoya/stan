...
d <- list(N = dim(df)[1],
          ...
          X = df$score,
          team = as.numeric(df$team),
          gf = as.numeric(df$gf))
d
fit <- stan(file = "poisson.stan", data=d, iter = 4000)
fit

r.fr <- rstan::extract(fit)$r[,11]
r.cr <- rstan::extract(fit)$r[,7]
beta1 <- rstan::extract(fit)$beta[,1]
beta2 <- rstan::extract(fit)$beta[,2]

lambda.fr <- exp(beta1 + r.fr + beta2)
lambda.cr <- exp(beta1 + r.cr + beta2)

mean(dpois(x = 0, lambda = lambda.cr))
mean(dpois(x = 1, lambda = lambda.cr))
mean(dpois(x = 2, lambda = lambda.cr))
mean(dpois(x = 3, lambda = lambda.cr))
mean(dpois(x = 4, lambda = lambda.cr))
mean(dpois(x = 5, lambda = lambda.cr))

mean(dpois(x = 0, lambda = lambda.fr))
mean(dpois(x = 1, lambda = lambda.fr))
mean(dpois(x = 2, lambda = lambda.fr))
mean(dpois(x = 3, lambda = lambda.fr))
mean(dpois(x = 4, lambda = lambda.fr))
mean(dpois(x = 5, lambda = lambda.fr))
